...
public class RatingsRepo {
    ...
    public void createRating(Rating rating, Attraction attraction) {
        var updated = jdbcClient.sql("INSERT INTO Ratings(userName, rating, comment, attraction_id, date) VALUES(?, ?, ?, ?, ?)")
                .params(List.of(rating.getUserName(), rating.getRating(), rating.getComment(), attraction.getId(), rating.getDate()))
                .update();
    ...
    public void createRatingDefDate(Rating rating, Attraction attraction) {
        var updated = jdbcClient.sql("INSERT INTO Ratings(userName, rating, comment, attraction_id) VALUES(?, ?, ?, ?)")
                .params(List.of(rating.getUserName(), rating.getRating(), rating.getComment(), attraction.getId()))
                .update();
